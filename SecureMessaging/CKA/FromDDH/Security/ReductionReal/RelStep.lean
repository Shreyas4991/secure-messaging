...
/-!
# CKA from DDH — Reduction (real) — State Relation and Per-Query Step
...
the relation `R sR sH` synchronizes counters, last action, public messages,
keys, and corruption-visible local states between `sR` and `sH`.

The step theorem proves, for each oracle query `t`,

```lean
RelTriple
  ((reductionOracleImpl gp gen
      (a • gen) (b • gen) ((a * b) • gen) t).run sR)
  ((honestImpl_param_real gp gen a b t).run sH)
...
`ReductionReal/Bridge.lean` uses this as the inductive step for `simulateQ`:
...
omit [Fintype G] in
/-- Real-branch simulation invariant between reduction and honest states. -/
def reductionHonestRel (gp : GameParams) (gen : G) (a b : F)
    (sR sH : GameState (CKAState F G) G G) : Prop :=
  -- The honest-side state has the protocol shape reachable from generator `gen`.
  reachableShape gen sH ∧
  -- The two states agree on counters, last action, transcript cells, and keys.
  sR.tA = sH.tA ∧
  sR.tB = sH.tB ∧
  ...
  sR.keyA = sH.keyA ∧
  sR.keyB = sH.keyB ∧
  -- At the initial action marker, both epoch counters are zero.
  (sR.lastAction = none → sR.tA = 0 ∧ sR.tB = 0) ∧
  -- A's local store either agrees, or is one of the two allowed dead-store cases.
  (sR.stA = sH.stA ∨
  ...
  -- B's local store has the symmetric agreement-or-dead-store alternatives.
  (sR.stB = sH.stB ∨
  ...
  -- In these A-side control states, the store disjunction must use the `0`/`a` branch.
  (gp.challengedParty = .B → sR.tA = gp.challengeEpoch - 1 →
    (sR.lastAction = some .sendA ∨ sR.lastAction = some .challA ∨
      sR.lastAction = some .recvB) →
    sR.stA = (.recvReady 0 : CKAState F G) ∧ sH.stA = .recvReady a) ∧
  -- In these B-side control states, the store disjunction must use the `0`/`a` branch.
  (gp.challengedParty = .A → sR.tB + 1 = gp.challengeEpoch →
    ...
    sR.stB = (.recvReady 0 : CKAState F G) ∧ sH.stB = .recvReady a) ∧
  -- The PCS gate, or A's FS gate, rules out an A-side dead store.
  ((allowCorrPCS gp sR || allowCorrFS gp sR .A) = true → sR.stA = sH.stA) ∧
  -- The PCS gate, or B's FS gate, rules out a B-side dead store.
...
/-- Preservation of `reductionHonestRel` by a successful `recvB` step.
...
private lemma reductionHonestRel_after_recvB
    (gp : GameParams) (a b y : F) (cR cH : Bool)
    ...
    (h_rhoB : sR.rhoB = sH.rhoB) (h_keyB : sR.keyB = sH.keyB)
    (h_stA : sR.stA = sH.stA ∨
      (gp.challengedParty = .B ∧ sR.tA = gp.challengeEpoch - 1 ∧
        sR.stA = (.recvReady 0 : CKAState F G) ∧ sH.stA = .recvReady a) ∨
      (gp.challengedParty = .A ∧ sR.tA = gp.challengeEpoch ∧
...
omit [Fintype F] [DecidableEq F] [SampleableType F] [SampleableType G]
  [DecidableEq G] [Inhabited F] [Fintype G] in
/-- Preservation of `reductionHonestRel` by a successful `recvA` step.
...
private lemma reductionHonestRel_after_recvA
    (gp : GameParams) (a b x : F) (cR cH : Bool)
    (sR sH : GameState (CKAState F G) G G)
    ...
    (h_tA : sR.tA = sH.tA) (h_tB : sR.tB = sH.tB)
    (h_rhoA : sR.rhoA = sH.rhoA) (h_keyA : sR.keyA = sH.keyA)
    (h_stB : sR.stB = sH.stB ∨
      (gp.challengedParty = .A ∧ sR.tB = gp.challengeEpoch - 1 ∧
        sR.stB = (.recvReady 0 : CKAState F G) ∧ sH.stB = .recvReady a) ∨
      (gp.challengedParty = .B ∧ sR.tB = gp.challengeEpoch ∧
...
/-- Per-query preservation for the real-branch `corruptB` oracle case. -/
private lemma reduction_honest_param_real_corruptB_rel
    (gp : GameParams) (a b : F)
    (sR sH : GameState (CKAState F G) G G)
    (h_tA : sR.tA = sH.tA) (h_tB : sR.tB = sH.tB)
    (h_safeB : (allowCorrPCS gp sR || allowCorrFS gp sR .B) = true →
      sR.stB = sH.stB)
    (hrel : reductionHonestRel gp gen a b sR sH) :
    OracleComp.ProgramLogic.Relational.RelTriple
      ((reductionOracleImpl gp gen (a • gen) (b • gen) ((a * b) • gen)
        OCorruptB).run sR)
      ((honestImpl_param_real gp gen a b OCorruptB).run sH)
      (fun pR pH => pR.1 = pH.1 ∧ reductionHonestRel gp gen a b pR.2 pH.2) := by
  simp only [reductionOracleImpl, honestImpl_param_real, QueryImpl.add_apply_inl,
    QueryImpl.add_apply_inr]
  exact relTriple_oracleCorruptB_of_state_rel
    (gp := gp)
    (R := fun sR sH => reductionHonestRel gp gen a b sR sH)
    (sL := sR) (sR := sH) h_tA h_tB h_safeB hrel

omit [Inhabited F] [Fintype G] in
/-- Per-query preservation for the real-branch `corruptA` oracle case. -/
private lemma reduction_honest_param_real_corruptA_rel
    (gp : GameParams) (a b : F)
    (sR sH : GameState (CKAState F G) G G)
    (h_tA : sR.tA = sH.tA) (h_tB : sR.tB = sH.tB)
    (h_safeA : (allowCorrPCS gp sR || allowCorrFS gp sR .A) = true →
    ...
    OracleComp.ProgramLogic.Relational.RelTriple
      ((reductionOracleImpl gp gen (a • gen) (b • gen) ((a * b) • gen)
        OCorruptA).run sR)
      ((honestImpl_param_real gp gen a b OCorruptA).run sH)
      (fun pR pH => pR.1 = pH.1 ∧ reductionHonestRel gp gen a b pR.2 pH.2) := by
  simp only [reductionOracleImpl, honestImpl_param_real, QueryImpl.add_apply_inl,
    QueryImpl.add_apply_inr]
...
omit [Inhabited F] [Fintype G] in
/-- Per-query preservation for the real-branch uniform oracle case. -/
private lemma reduction_honest_param_real_unif_rel
    (gp : GameParams) (a b : F) (n : unifSpec.Domain)
    (sR sH : GameState (CKAState F G) G G)
    (hrel : reductionHonestRel gp gen a b sR sH) :
    OracleComp.ProgramLogic.Relational.RelTriple
      ((reductionOracleImpl gp gen (a • gen) (b • gen) ((a * b) • gen)
        (OUnif n)).run sR)
      ((honestImpl_param_real gp gen a b (OUnif n)).run sH)
      (fun pR pH => pR.1 = pH.1 ∧ reductionHonestRel gp gen a b pR.2 pH.2) := by
  simp only [reductionOracleImpl, honestImpl_param_real, QueryImpl.add_apply_inl]
...
omit [Inhabited F] [Fintype G] in
/-- Per-query preservation of the real-branch relation `reductionHonestRel`. -/
lemma reduction_honest_param_real_step_rel
  (gp : GameParams) (hΔFS : gp.ΔFS = 1) (hΔPCS : gp.ΔPCS = 2)
    ...
    OracleComp.ProgramLogic.Relational.RelTriple
      ...
      (fun pR pH => pR.1 = pH.1 ∧ reductionHonestRel gp gen a b pR.2 pH.2) := by
  rcases hrel with
    ⟨h_shape, h_tA, h_tB, h_last, h_rhoA, h_rhoB, h_keyA, h_keyB,
      h_none, h_stA, h_stB, h_pendingA,
      h_pendingB, h_safeA, h_safeB⟩
  have hrel_self : reductionHonestRel gp gen a b sR sH := by
    exact
      ⟨h_shape, h_tA, h_tB, h_last, h_rhoA, h_rhoB, h_keyA, h_keyB,
        h_none, h_stA, h_stB, h_pendingA,
        h_pendingB, h_safeA, h_safeB⟩
  match t with
  | OCorruptB =>
      exact reduction_honest_param_real_corruptB_rel
        (gen := gen) gp a b sR sH h_tA h_tB h_safeB hrel_self
  | OCorruptA =>
      exact reduction_honest_param_real_corruptA_rel
        (gen := gen) gp a b sR sH h_tA h_tB h_safeA hrel_self
  | OChallB =>
      simp only [reductionOracleImpl, honestImpl_param_real, QueryImpl.add_apply_inl,
        QueryImpl.add_apply_inr]
        ...
        · have h_vH : validStep sH.lastAction CKAAction.challB = true := by
          ...
          · have h_epochH :
            ...
            all_goals
              ...
              subst y
              have h_key_eq : b • (a • gen) = (a * b) • gen := by
                rw [smul_smul, mul_comm]
              have h_runR :
                  (reductionChallB (F := F) gp (b • gen) ((a * b) • gen) ()).run sR =
                    (pure (some (b • gen, (a * b) • gen),
                      { sR with
                        tB := sR.tB + 1, stB := (.recvReady 0 : CKAState F G),
                        rhoB := some (b • gen), keyB := some ((a * b) • gen),
              ...
              have h_runH :
                  (honestChallB_param (F := F) gp gen b ()).run sH =
                    (pure (some (b • gen, (a * b) • gen),
                      { sH with
                        ...
                        lastAction := some .challB }) :
                      ProbComp (Option (G × G) × GameState (CKAState F G) G G)) := by
                unfold honestChallB_param
                rw [StateT.run_get_bind]
                simp [h_lastH, validStep, h_cp, h_epochH_nat, h_stBH,
                  h_key_eq, isChallengeEpoch, GameState.tP]
              refine relTriple_of_eq_pure_pure
                (oa := (reductionChallB (F := F) gp (b • gen) ((a * b) • gen) ()).run sR)
                (ob := (honestChallB_param (F := F) gp gen b ()).run sH)
                (R := fun pR pH =>
                  pR.1 = pH.1 ∧ reductionHonestRel gp gen a b pR.2 pH.2)
                (a := (some (b • gen, (a * b) • gen),
                  { sR with
                    tB := sR.tB + 1, stB := (.recvReady 0 : CKAState F G),
                    rhoB := some (b • gen), keyB := some ((a * b) • gen),
                    lastAction := some .challB }))
                (b := (some (b • gen, (a * b) • gen),
                  { sH with
                    tB := sH.tB + 1, stB := (.recvReady b : CKAState F G),
                    rhoB := some (b • gen), keyB := some ((a * b) • gen),
                    lastAction := some .challB }))
          ...
          · have h_epochR :
            ...
            exact relTriple_reductionChallB_pred_false_of_state_rel
              (mode := .real) (gp := gp) (gen := gen) (b := b)
              (gB := b • gen) (gT := (a * b) • gen) (gTH := gen)
              (R := fun sR sH => reductionHonestRel gp gen a b sR sH)
              (sL := sR) (sR := sH)
          ...
          · have h_epochH :
            ...
            all_goals
                  ...
                  (reductionChallA (F := F) gp (b • gen) ((a * b) • gen) ()).run sR =
                    (pure (some (b • gen, (a * b) • gen),
                      { sR with
                        ...
                        rhoA := some (b • gen), keyA := some ((a * b) • gen),
              ...
              have h_epochH_nat : sH.tA + 1 = gp.challengeEpoch := by
                simpa [isChallengeEpoch, GameState.tP, h_cp] using h_epochH
              have h_runH :
                  (honestChallA_param (F := F) gp gen b ()).run sH =
                    (pure (some (b • gen, (a * b) • gen),
                      { sH with
                        ...
                        lastAction := some .challA }) :
                      ProbComp (Option (G × G) × GameState (CKAState F G) G G)) := by
                unfold honestChallA_param
                rw [StateT.run_get_bind]
                simp [h_lastH, validStep, h_cp, h_epochH_nat, h_stAH,
                  h_key_eq, isChallengeEpoch, GameState.tP]
              refine relTriple_of_eq_pure_pure
                (oa := (reductionChallA (F := F) gp (b • gen) ((a * b) • gen) ()).run sR)
                (ob := (honestChallA_param (F := F) gp gen b ()).run sH)
                (R := fun pR pH =>
                  pR.1 = pH.1 ∧ reductionHonestRel gp gen a b pR.2 pH.2)
                (a := (some (b • gen, (a * b) • gen),
                  { sR with
      ...
      · have h_vH : validStep sH.lastAction CKAAction.recvB = true := by
        ...
        all_goals
          ...
          have h_rhoAR : sR.rhoA = some (y • gen) := by
            rw [h_rhoA, h_rhoAH]
          have h_keyAR : sR.keyA = some (y • (x • gen)) := by
            rw [h_keyA, h_keyAH]
          let cR : Bool :=
          ...
          have h_runR :
            ...
            unfold oracleRecvB
            rw [StateT.run_get_bind]
            simp [h_v, h_stBR, h_rhoAR, h_keyAR, ddhCKA, recv, cR]
          have h_runH :
          ...
          refine relTriple_of_eq_pure_pure
            ...
            (b := ((), { sH with
              ...
              lastAction := some .recvB }))
            h_runR h_runH ?_
          exact ⟨rfl, reductionHonestRel_after_recvB (gen := gen) gp a b y cR cH
            sR sH h_phase h_stAH h_rhoBH h_keyBH h_tA h_tB h_rhoB h_keyB
            h_stA
            (by
      ...
      · have h_vH : validStep sH.lastAction CKAAction.recvA = true := by
        ...
        all_goals
          ...
          have h_rhoBR : sR.rhoB = some (x • gen) := by
            rw [h_rhoB, h_rhoBH]
          have h_keyBR : sR.keyB = some (x • (y • gen)) := by
            rw [h_keyB, h_keyBH]
          let cR : Bool :=
          ...
          have h_runR :
            ...
            unfold oracleRecvA
            rw [StateT.run_get_bind]
            simp [h_v, h_stAR, h_rhoBR, h_keyBR, ddhCKA, recv, cR]
          have h_runH :
          ...
          refine relTriple_of_eq_pure_pure
            ...
            (b := ((), { sH with
              ...
              lastAction := some .recvA }))
            h_runR h_runH ?_
          exact ⟨rfl, reductionHonestRel_after_recvA (gen := gen) gp a b x cR cH
            sR sH h_phase h_stBH h_rhoAH h_keyAH h_tA h_tB h_rhoA h_keyA
            h_stB
            (by
  ...
  | OSendA_rleak =>
        ...
        · have h_allowH : allowCorrPCS gp {sH with tA := sH.tA + 1} = true := by
          ...
          all_goals
            ...
            have h_stAR : sR.stA = (.sendReady (x • gen) : CKAState F G) := by
              rw [h_stA_eq, h_stAH]
            have h_allowR' :
                allowCorrPCS gp {sR with
                  tA := sR.tA + 1, stA := (.sendReady (x • gen) : CKAState F G)} = true := by
              simpa [allowCorr, allowCorrPCS] using h_allow
            have h_allowH' :
                allowCorrPCS gp {sH with
                  tA := sH.tA + 1, stA := (.sendReady (x • gen) : CKAState F G)} = true := by
              simpa [allowCorr, allowCorrPCS] using h_allowH
            have h_runR :
      ...
      · exact relTriple_oracleSendA_rleak_invalid_of_state_rel
          ...
          (sL := sR) (sR := sH) h_last h_v hrel_self
  | OSendB_rleak =>
      simp only [reductionOracleImpl, honestImpl_param_real, QueryImpl.add_apply_inr]
      by_cases h_v : validStep sR.lastAction CKAAction.sendB = true
      · have h_vH : validStep sH.lastAction CKAAction.sendB = true := by
          rw [← h_last]
          exact h_v
      ...
      · exact relTriple_oracleSendB_rleak_invalid_of_state_rel
          ...
          (sL := sR) (sR := sH) h_last h_v hrel_self
  | OUnif n =>
      exact reduction_honest_param_real_unif_rel
        (gen := gen) gp a b n sR sH hrel_self

omit [Inhabited F] [Fintype G] in
/-- Fixed-scalar reduction stack is observationally equivalent to the
parameterized honest stack.

This is the dead-store part of the real-branch bridge: the reduction writes
`.recvReady 0` into party state at embedding/challenge events, while
`honestImpl_param_real` writes the cached scalar. Those fields are not
observable before the protocol overwrites them again; corruption is delayed
by `hΔFS` and `hΔPCS`. -/
lemma evalDist_reduction_honest_param_real_eq
  (gp : GameParams) (hΔFS : gp.ΔFS = 1) (hΔPCS : gp.ΔPCS = 2)
  (h_general_case : ¬ (gp.challengeEpoch = 1 ∧ gp.challengedParty = .A))
    ...
    evalDist ((simulateQ (honestImpl_param_real gp gen a b) adversary).run'
      (initGameState
        (CKAState.sendReady (x₀ • gen) : CKAState F G)
...
omit [Inhabited F] [Fintype G] in
/-- **Reduction–honest bridge.**

For any adversary, the lazy reduction simulation's output distribution
equals the honest CKA simulation's. This is the whole-adversary form of the
real-branch reduction/honest equivalence. -/
lemma evalDist_eager_reduction_lazy_eq
  (gp : GameParams) (hΔFS : gp.ΔFS = 1) (hΔPCS : gp.ΔPCS = 2)
  (h_general_case : ¬ (gp.challengeEpoch = 1 ∧ gp.challengedParty = .A))
    (x₀ : F)
    (adversary : OracleComp (ckaSecuritySpec (CKAState F G) G G F) Bool) :
    ...
    evalDist ((simulateQ (ckaSecurityImpl gp false (ddhCKA F G gen)) adversary).run'
      (initGameState
        (CKAState.sendReady (x₀ • gen) : CKAState F G)
        (CKAState.recvReady x₀ : CKAState F G))) := by
  let s₀ : GameState (CKAState F G) G G :=
    initGameState
      (CKAState.sendReady (x₀ • gen) : CKAState F G)
      (CKAState.recvReady x₀ : CKAState F G)
  calc
